...
class RelatedSection(Template.Section):
    ...
    def makeLink(self, path, title):
        ...
        target = cia.LibCIA.Stats.Target.StatsTarget(path)
        if title is None:
            title = target.name
...
class GraphPage(resource.Resource):
    ...
    def __init__(self, *args, **kwargs):
        self.grapher = cia.LibCIA.Stats.Graph.RelationGrapher(*args, **kwargs)
    ...
    def format_dot(self, request):
        ...
        return cia.LibCIA.Stats.Graph.RenderCache(self.grapher)
    ...
    def format_svg(self, request):
        ...
        graphCache = cia.LibCIA.Stats.Graph.RenderCache(self.grapher)
        layout = cia.LibCIA.Stats.Graph.GraphLayout(graphCache)
        return cia.LibCIA.Stats.Graph.RenderCache(layout)
    ...
    def format_png(self, request, defaultWidth=600, maxWidth=2000):
        ...
        graphCache = cia.LibCIA.Stats.Graph.RenderCache(self.grapher)
        layoutCache = cia.LibCIA.Stats.Graph.RenderCache(cia.LibCIA.Stats.Graph.GraphLayout(graphCache))
        raster = cia.LibCIA.Stats.Graph.SvgRasterizer(layoutCache, width=width, background=bg)
        return cia.LibCIA.Stats.Graph.RenderCache(raster)
