...
{var image=obj.getElementsByTagName("img")[0];if(HasClass(obj,"expander-closed")){KillClass(obj,"expander-closed");AddClass(obj,"expander-open");image.src="/media/img/tlexpand-open.png";return true;}else{AddClass(obj,"expander-closed");KillClass(obj,"expander-open");image.src="/media/img/tlexpand-closed.png";return false;}}
var animateMessages=function(id,visible)
{var parent_div=document.getElementById(id);var content_div=parent_div.getElementsByTagName("div")[0]
if(parent_div.currentAnim){parent_div.currentAnim.stop();}
var resize=new YAHOO.util.Anim(parent_div);if(visible){resize.attributes.height={from:0,to:parent_div.originalHeight};}else{parent_div.originalHeight=parent_div.offsetHeight;resize.attributes.height={from:parent_div.originalHeight,to:0};resize.setAttribute('height',parent_div.originalHeight,'px')
content_div.style.display="none";}
resize.onComplete.subscribe(function(){if(visible){content_div.style.display="block";parent_div.style.height="";}
parent_div.currentAnim=null;});resize.duration=0.5;resize.method=YAHOO.util.Easing.easeOut;parent_div.currentAnim=resize;resize.animate();}
var animateExpander=function(id,visible)
{var div=document.getElementById(id);var fade=new YAHOO.util.Anim(div);if(div.currentAnim){div.currentAnim.stop();}
if(visible){div.style.display="inline";fade.attributes.opacity={from:0,to:1};}else{fade.attributes.opacity={from:1,to:0};}
fade.onComplete.subscribe(function(){if(!visible){div.style.display="none";}
div.currentAnim=null;});fade.duration=0.25;fade.method=YAHOO.util.Easing.easeOut;div.currentAnim=fade;fade.animate();}
var expandDay=function(y,m,d)
{var visible=toggleExpander(document.getElementById('expander-'+y+'-'+m+'-'+d));animateMessages('msg-'+y+'-'+m+'-'+d,visible);}
var expandMonth=function(y,m,d)
{var visible=toggleExpander(document.getElementById('expander-'+y+'-'+m));animateMessages('msg-'+y+'-'+m,visible);animateExpander('expander-'+y+'-'+m+'-'+d,visible);}
var expandYear=function(y,m,d)
{var visible=toggleExpander(document.getElementById('expander-'+y));animateMessages('msg-'+y,visible);animateExpander('expander-'+y+'-'+m+'-'+d,visible);animateExpander('expander-'+y+'-'+m,visible);}
