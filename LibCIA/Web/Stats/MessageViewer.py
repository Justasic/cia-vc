...
class EnvelopeSection(Template.Section):
    ...
    def _render_rows(self, xml, context, result):
        ...
        if m.xml.timestamp:
            rows.append(self.format_timestamp(m.xml.timestamp))
        if m.xml.generator:
            rows.append(self.format_generator(m.xml.generator))

        result.callback(rows)

    def format_generator(self, gen):
        """Format the information contained in this message's <generator> tag"""
        name = str(gen.__getattr__('name')).strip()
        if gen.url:
            name = tag('a', href=str(gen.url).strip())[ name ]
        items = ["Generated by ", Template.value[name]]
        if gen.version:
            items.extend([" version ", str(gen.version).strip()])
        return items

    def format_timestamp(self, stamp):
        return ["Received at ", Template.value[TimeUtil.formatDate(int(str(stamp)))]]
